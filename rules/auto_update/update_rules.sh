...
get_chnroute(){
	...
	# 6. write version
	sed -i "2c $(date +%Y-%m-%d) # $md5sum1 chnroute" ../version1
}
...
get_cdn(){
	...
	# 6. write version
	sed -i "4c $(date +%Y-%m-%d) # $md5sum1 cdn" ../version1
}
...
get_apple(){
	...
	# 5. write version
	sed -i "8c $(date +%Y-%m-%d) # $md5sum1 apple_china" ../version1
}
...
get_google(){
	...
	# 5. write version
	sed -i "9c $(date +%Y-%m-%d) # $md5sum1 google_china" ../version1
}
...
get_white(){
	...
	local md5sum1=$(md5sum WhiteList.txt | sed 's/ /\n/g' | sed -n 1p)
	local md5sum2=$(md5sum ../WhiteList.txt | sed 's/ /\n/g' | sed -n 1p)
...
get_whitenew(){
	...
	local md5sum1=$(md5sum WhiteList_new.txt | sed 's/ /\n/g' | sed -n 1p)
	local md5sum2=$(md5sum ../WhiteList_new.txt | sed 's/ /\n/g' | sed -n 1p)
